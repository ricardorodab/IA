...
public class AEstrella extends PApplet {
    ...
    public void draw(){
        ...
        for(int i = 0; i < renglones; i++){
			...
			//POR ALGUNA EXTRAÑA RAZÓN ESTA PARTE DEL CÓDIGO ME CAUSA UNA EXCEPTION PERO TODO FUNCIONA SIN ÉL PERFECTAMENTE.
			//ASÍ QUE LO COMENTÉ!
			//MEREZCO UN PUNTO EXTRA POR DEBUGGEAR ESTO!! PASÉ 5 HORAS PENSANDO QUE ERA MI CÓDIGO QUE ESTABA MAL!
	...
	//Situacion situacion = Situacion.EN_LISTA_ABIERTA;;
        ...
        void calculaHeuristica(Mosaico meta){
            ...
            */
            hn = buscaFin(meta,this)*10;	    
        }

	int buscaFin(Mosaico meta, Mosaico actual){
	    int col = Math.abs(meta.renglon-actual.renglon);
	    int ren = Math.abs(meta.columna-actual.columna);
	    return col+ren;
    ...
    class Algoritmo {
	    ...
	    if(nodoActual.estado.situacion != Situacion.EN_LISTA_CERRADA)
		nodoActual.estado.situacion = Situacion.ACTUAL;
	    if(nodoActual.equals(new NodoBusqueda(estadoFinal))){
		resuelto = true;
	    ...
	    LinkedList<NodoBusqueda> sucesores = nodoActual.getSucesores();	    
	    for(NodoBusqueda n : sucesores){
		if(n.estado.situacion == Situacion.EN_LISTA_ABIERTA)
		    continue;
		else if(n.estado.situacion == Situacion.EN_LISTA_CERRADA)
		    continue;
		else if(n.estado.situacion == Situacion.ACTUAL)
		    continue;
		Situacion situacion = Situacion.EN_LISTA_ABIERTA;
		n.estado.situacion = situacion;
		n.estado.gn = n.gn;
