...
/**
 *
 * @author Verónica Arriola y Rodríguez Abreu José ricardo
 */
public class Gatos extends PApplet {
    ...
    public void setup(){
        ...
        anchoImagen = 100+30;//anchoGato; //ANCHOSTAGE
        altoImagen =  altoGato*10;//altoGato; //ALTOSTAGE
    ...
    class Gato{
        ...
        */
	    ...
	    if(hayGanador)
		return new LinkedList<Gato>();
	    this.sucesores = new LinkedList<Gato> ();
	    for(int i = 0; i < 3; i++){
		for(int j = 0; j < 3; j++){
		    Gato nuevo = new Gato(this);
		    if(nuevo.tablero[i][j] == MARCA1 || nuevo.tablero[i][j] == MARCA2)
			continue;		    
		    nuevo.padre = this;
		    ...
		    for(int k = 0; k < this.sucesores.size(); k++){
			if(nuevo.equals(sucesores.get(k))){
			    existe = true;
			    break;
			}			
		    }
		    if(!existe)
			this.sucesores.add(nuevo);
		}
	    }
